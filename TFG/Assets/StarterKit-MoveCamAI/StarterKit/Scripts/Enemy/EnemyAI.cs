...
public class EnemyAI : MonoBehaviour {
	...
	void EnemyMove()
	{
		if(playerSpotted) // If the player has been spotted then continue
		{
			if(Vector3.Distance(transform.position,player.position) > 2) // If the distance from the player is greater than a number then continue
			{
				...
				//rigidbody.MovePosition(Vector3.MoveTowards(transform.position, player.position, Time.deltaTime * speed)); // Move the enemy towards the players position
                GetComponent<Rigidbody>().MovePosition(Vector3.MoveTowards(transform.position, player.position, Time.deltaTime * speed));
		...
		else if(FindLastPosition() != null) // If the player has NOT been spotted and a last position has been found then continue **FindLastPosition() returns a GameObject
		{
			...
			//rigidbody.MovePosition(Vector3.MoveTowards(transform.position, FindLastPosition().transform.position, Time.deltaTime * speed)); // Move the enemy towards the found last position game object
	...
	{
		//rigidbody.MoveRotation(Quaternion.LookRotation(position - transform.position)); // Rotate the enemy to look towards the players position
        GetComponent<Rigidbody>().MoveRotation(Quaternion.LookRotation(position - transform.position));
    }
	...
	private void EnemyAvoid()
	{
		...
		if (Physics.Raycast(transform.position,-transform.right,out left,avoidDist, ~(lastPosLayer | enemyLayer | playerLayer)))
		{
			...
			//rigidbody.AddForce(transform.right * avoidSpeed);   // Add force to the right
            GetComponent<Rigidbody>().AddForce(transform.right * avoidSpeed);
        } 
		else // If the cast no is no longer hitting anything the continue
		{
			//rigidbody.velocity = Vector3.zero; // Stops all rigidbody movement, this removes any sliding around
		...
		if (Physics.Raycast(transform.position,transform.right,out right,avoidDist, ~(lastPosLayer | enemyLayer | playerLayer)))
		{
			...
			//rigidbody.AddForce(-transform.right * avoidSpeed); // Add force to the left
            GetComponent<Rigidbody>().AddForce(-transform.right * avoidSpeed);
        } 
		else // If the cast no is no longer hitting anything the continue
		{
